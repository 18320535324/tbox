...
public class DatabaseMetaDateApplication {
	...
	Connection con = null;
	private static DatabaseMetaDateApplication databaseMetaDateApplication=null;

	public static void init(String jdbcDriver,String jdbcUrl,String jdbcUser,String jdbcPasswd){
		if(jdbcDriver==null){
			throw new RuntimeException("jdbcDriver is not null");
		}
		if(jdbcUrl==null){
			throw new RuntimeException("jdbcUrl is not null");
		}
		if(jdbcUser==null){
			throw new RuntimeException("jdbcUser is not null");
		}
		if(jdbcPasswd==null){
			throw new RuntimeException("jdbcPasswd is not null");
		}
		properties.put("jdbcDriver",jdbcDriver);
		properties.put("jdbcUrl",jdbcUrl);
		properties.put("jdbcUser",jdbcUser);
		properties.put("jdbcPasswd",jdbcPasswd);
		databaseMetaDateApplication=new DatabaseMetaDateApplication();
		databaseMetaDateApplication.getDatabaseMetaData();
	}

	public static DatabaseMetaDateApplication get(){
		if(databaseMetaDateApplication==null){
			throw new RuntimeException("databaseMetaDateApplication 未初始化，请调用：DatabaseMetaDateApplication.init方法进行初始化");
		}
		return databaseMetaDateApplication;
	}

	private DatabaseMetaDateApplication(){}

	private void getDatabaseMetaData() {
		try {
			if (dbMetaData == null) {
				String jdbcDriver = properties.get("jdbcDriver");
				String jdbcUrl = properties.get("jdbcUrl");
				String jdbcUser = properties.get("jdbcUser");
				String jdbcPasswd = properties.get("jdbcPasswd");
				con = DBUtil.getConnection(jdbcDriver, jdbcUrl, jdbcUser, jdbcPasswd);
	...
	public Table getTableColumns(String schemaName, String tableName) {
		...
		try {
			...
			while (rs.next()) {
				...
				int nullAble = rs.getInt("NULLABLE");// 是否允许为null (约束为0是not null，1是Null)
				...
				column.type = dataTypeName;
				column.desc = remarks;
				column.wasNull = nullAble == 1 ? true : false;
...
//		List<String> tables = metaData.getAllTableList("root");
//		for (String tableName : tables) {
//			System.out.println(tableName);
//			String sql = "delete from " + tableName + " where is_deleted =1";
//			metaData.con.createStatement().execute(sql);
//		}
