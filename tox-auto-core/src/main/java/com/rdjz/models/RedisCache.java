...
public class RedisCache implements CacheService {
    ...
    public List<Object> lrange(String key,long start,long end){
        ...
        try {
            jedis = pool.getResource();
            List<byte[]>  datas =  jedis.lrange(key.getBytes(),start,end);
            if(datas == null) return null;
            return SerializeUtil.unserializes(datas);
        }catch (Exception e) {
            logger.error(e.getMessage(), e);
            pool.returnBrokenResource(jedis);
            throw new RuntimeException("Redis出现错误！" + e.getMessage(), e);
    ...
    public Object rpoplpush(String source,String destination){
        ...
        try {
            jedis = pool.getResource();
            byte[]  data =  jedis.rpoplpush(source.getBytes(),destination.getBytes());
            if(data == null) return null;
            return SerializeUtil.unserialize(data);
        }catch (Exception e) {
            logger.error(e.getMessage(), e);
    ...
    public Long sadd(String key, String member, int second) {
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
...
//    private boolean locked = false;
    ...
    public void unlock(String key) {
        ...
        try {
            jedis.del(key);
            logger.info( "釋放锁.key:" + key);
        } finally {
            pool.returnResource(jedis);
        }
    }

    private boolean lock(Jedis jedis, String key, long timeout) {
        return lock(jedis, key, timeout, EXPIRE);
    }

    private boolean lock(Jedis jedis, String key, long timeout, int expiredTime) {
        long nanoTime = System.nanoTime();
        timeout *= ONE_MILLI_NANOS;
        try {
            logger.info( "嘗試获得锁.key:" + key);
